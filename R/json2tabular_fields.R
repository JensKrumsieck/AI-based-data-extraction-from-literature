...
input_folder <- paste(opt$input_folder, "fields", sep="/")
output_folder <- opt$output_folder


# ── 1. Configuration ──────────────────────────────────────────────────────────
#input_folder  <- "C:\\Users\\xinxin\\OneDrive - Leibniz-Zentrum für Agrarlandschaftsforschung (ZALF) e.V\\Desktop\\FAIRagro_fine_tunning\\data\\0_training_set_version2\\gpt-4.1-mini_finetuning_output_v010526_fields_v95"          # folder containing your JSON/txt files
#output_folder <- "C:\\Users\\xinxin\\OneDrive - Leibniz-Zentrum für Agrarlandschaftsforschung (ZALF) e.V\\Desktop\\FAIRagro_fine_tunning\\data\\0_training_set_version2\\gpt-4.1-mini_finetuning_output_v010526_fields_v95_excel"  # folder where Excel files will be saved
...
convert_json_to_excel <- function(json_path, out_path) {
  ...
  if (!is.null(data$cropYear$FIELDS)) {
    # Combine the list of observations into one flat dataframe
    df <- bind_rows(data$cropYear$FIELDS)
  } else if (!is.null(data$FIELDS)) {
    df <- as.data.frame(data$FIELDS)
  } else {
  ...
  df <- bind_rows(lapply(data$cropYear$FIELDS, function(rec) {
