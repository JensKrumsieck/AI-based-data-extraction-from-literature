...
# ── 1. Configuration ──────────────────────────────────────────────────────────
#input_folder  <- "C:\\Users\\xinxin\\OneDrive - Leibniz-Zentrum für Agrarlandschaftsforschung (ZALF) e.V\\Desktop\\FAIRagro_fine_tunning\\data\\0_training_set_version2\\gpt-4.1-mini_finetuning_output_v050526_planting_v106"          # folder containing your JSON/txt files
#output_folder <- "C:\\Users\\xinxin\\OneDrive - Leibniz-Zentrum für Agrarlandschaftsforschung (ZALF) e.V\\Desktop\\FAIRagro_fine_tunning\\data\\0_training_set_version2\\gpt-4.1-mini_finetuning_output_v050526_planting_v106_excel"  # folder where Excel files will be saved
...
convert_json_to_excel <- function(json_path, out_path) {
  ...
  if (!is.null(data$cropYear$PLANTINGS)) {
    # Combine the list of observations into one flat dataframe
    df <- bind_rows(data$cropYear$PLANTINGS)
  } else if (!is.null(data$PLANTINGS)) {
    df <- as.data.frame(data$PLANTINGS)
  } else {
  ...
  all_cols <- c(
    ...
    "experiment_ID","planting_level_name","planting_date",
   "emergence_date","plant_pop_at_planting",
    "plant_pop_at_emergence","planting_material","planting_distribution",
    "plant_spacing","row_spacing", "planting_depth","planting_notes"
    
     )
  # ── Data Cleaning (Essential for Excel) ─────────────────────────────────────
  # Convert any remaining list-columns into comma-separated strings
  df <- bind_rows(lapply(data$cropYear$PLANTINGS, function(rec) {
